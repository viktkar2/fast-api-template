...
async def _seed():
    ...
    u1 = UserDocument(entra_object_id="user-001", display_name="Alice", email="alice@test.com")
    u2 = UserDocument(entra_object_id="user-002", display_name="Bob", email="bob@test.com")
    await u1.insert()
    await u2.insert()
    ...
    async def test_add_member_user_not_found(self, service):
        ...
        with pytest.raises(ValueError, match="user_not_found"):
            await service.add_member(
                data["group"].id, "nonexistent", GroupRole.USER
            )
...
class TestRemoveMember:
    async def test_remove_member_success(self, service):
        ...
        await service.add_member(
            data["group"].id, "user-002", GroupRole.ADMIN
        )
    ...
    async def test_remove_regular_user_when_only_one_admin(self, service):
        ...
        await service.add_member(
            data["group"].id, "user-002", GroupRole.USER
        )
